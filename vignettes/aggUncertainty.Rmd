...
#rmarkdown::render("vignettes/aggUncertainty.Rmd")
...
And hence its standard deviation by $sd(m) = \sqrt{Var(m)}= \sqrt{\sum{Var(x_i)}/n^2}$. 
This results in an aprroximate reduction of the standard deviation by $\sqrt{n}$.
...
the formulas now become $Var(m) = Var(\sum{x_i})/n_{eff}$ and
$\operatorname{Var}(\sum{x_i}) = \frac{n_{eff}}{n(n_{eff}-1)} \sum_{i=1}^n \sigma_i^2$,
with the number of effective observations $n_{eff}$ decreasing with the
autocorrelation among records (Bayley 1946, Zieba 2011).

The standard deviation now approximately decreases only by $\sqrt{n_{eff}}$.

First we need to quantify the correlations and inspect their autocorrelation.
```{r}
results <- EProc$sExportResults() %>% 
  mutate(
...
estimated on longer time series of residuals instead of 
estimating them each day.
...
aggDay %>% filter(between(DoY, 150, 200)) %>% 
  ggplot(aes(DateTime, NEE)) +
  geom_line() +
  geom_ribbon(aes(ymin = NEE - 1.96*sdNEEuncorr, ymax = NEE + 1.96*sdNEEuncorr), fill = "blue", alpha = 0.2) +
geom_ribbon(aes(ymin = NEE - 1.96*sdNEE, ymax = NEE + 1.96*sdNEE), fill = "orange", alpha = 0.2) +
  themeTw
```
