...
results_good <- results %>% filter(NEE_uStar_fqc == 0) 
summary(results_good$NEE_uStar_fsd)
```
We can inspect, how the uncertainty scales with the flux magnitude.
```{r}
plot( NEE_uStar_fsd ~ NEE_uStar_fall, 
      slice(results_good, sample.int(nrow(results_good),400)))
```

## Omitting problematic records
REddyProc flags problematic data with a qualtiy flag in `NEE_<uStar>_f` > 0 but 
still reports the fluxes. Hence, one should filter these records before
aggratation to exclude these data and their uncertainty from analysis. 
This can be done by `results %>% filter(NEE_uStar_fqc == 0)`.

Alternatively, one can explicitly set these values to missing.
```{r}
results_good2 <- EProc$sExportResults() %>% 
  mutate(
    NEE_uStar_f = ifelse(NEE_uStar_fqc == 0, NA, .data$NEE_uStar_f),
    NEE_uStar_fsd = ifelse(NEE_uStar_fqc == 0, NA, .data$NEE_uStar_fsd)
)
```
...
First we need to to compute `nEff` for each day on the unfiltered series
```{r}
aggDayNEff <- results %>% group_by(DoY) %>% 
  summarise(
...
For computing the mean and sd we filter for quality flag to avoid
compromising the mean with finite but bad-quality data.
```{r}
aggDay <- results %>% 
  filter(NEE_uStar_fqc == 0) %>% 
  # join the daily information of nEff from unfiltered data
  select(-"DateTime") %>% right_join(aggDayNEff, by = "DoY") %>% 
  group_by(DoY) %>% 
  summarise(
    DateTime = first(DateTime) # from join with aggDayNEff before
    , nEff = first(nEff)
    , nRec = sum(is.finite(NEE_uStar_f))
