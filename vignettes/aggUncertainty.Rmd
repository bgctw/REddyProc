...
REddyProc flags filled data with poor gap-filling by a qualtiy flag 
...
If the aggregated mean should be computed excluding poor quality-fap-filled
...
results %>% summarise(
  nRec = sum(is.finite(NEE_orig_sd))
  , NEE_uStar = mean(NEE_uStar_f, na.rm = TRUE)
  ...
  ) %>% select(NEE_uStar, seMean, seMeanApprox)
```
...
Now the aggretation can be done on data grouped by DoY. The notation `!!` tells
