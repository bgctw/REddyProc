...
**Relative humitity (rH)** in $\%$  was computed from AVP in $hPa$ 
and temperature.
...
```{r}
(potRad <- computePotentialRadiation(doy, hour, latDeg, longDeg, timezone))
```
...
This is replaced by bigleaf:
```{r}
(extRad <- computeExtRadiation(doy))
```
