...
Note that, here, the seaonson are not constrained within one calendaryear. 
...
```{r DEGEb_estUStar1a, spar=TRUE, fig.width=10}
	...
	seasonFactor <- usCreateSeasonFactorYdayYear(DEGebExample$DateTime, starts=seasonStarts)
	plot( NEE ~ DateTime, DEGebExample )
	seasonStartsDate <- fConvertTimeToPosix( data.frame(Year=seasonStarts[,2]
		, DoY=seasonStarts[,1], Hour=12),'YDH',Year.s="Year", Day.s="DoY",Hour.s="Hour")
...
With a lower estimate of uStar threshold, more records with lower NEE a kept in 
the dataset instead of marked as gaps. Therefore annual estimate of NEE will decrease
with decreasing uStar Threshold. Also the partitioning of the net-flux to GPP and REco is 
...
```{r DEGEb_estUStarBoot1, cache=TRUE, message=FALSE}
	...
	(UstarThres.df <- usGetSeaonalSeasonUStarMappingFromDistributionResult(uStarRes))
```	
...
In order to feed results of different uStar Threshold estimates into the Gap-Filling, the 
...
```{r DEGEb_fluxPart1, cache=TRUE}
	...
	for( suffix in c('U10', 'U90')){
		EddyProc.C$sMRFluxPartition(Lat_deg.n=51.1, Long_deg.n=10.9, TimeZone_h.n=1, Suffix.s = suffix)
	}
...
The user selects this method by specifying argument `ctrlUstarEst.l=usControlUstarEst(isUsingCPTSeveralT = TRUE)`
...
```{r DEGEb_estUStarCPT, cache=TRUE}
	...
	(UstarThresCP.df <- usGetSeaonalSeasonUStarMappingFromDistributionResult(resUStar))
	#UstarThres.df
```
