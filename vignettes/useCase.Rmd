...
The output reports uStarEstimates of `r signif(unlist(uStarTh[1,c(4)]),2)` for 
the orignal data and `r signif(unlist(uStarTh[1,c(5:7)]),2)` for lower, median, 
...
```{r fpNEEFilled, include = FALSE, eval = FALSE}
...
```
In this run of the example a relative error of about `r signif( (max(GPPAgg) - min(GPPAgg))/ median(GPPAgg)*100,2)`% is inferred.
