...
test_that("estimating temperature sensitivity oneWindow are in accepted range",{
			...
			dss$NEE <- dss$NEE_f
			resE0 <- REddyProc:::partGLEstimateTempSensInBoundsE0Only(dss$NEE_f, dss$Temp+273.15)
			...
			res <- REddyProc:::partGLFitNightRespRefOneWindow( dss, data.frame(iWindow=1L), E0Win=E0Win)
			RRef <- res[1]
			expect_true( RRef >= 0)
			.tmp.plot <- function(){
...
test_that("estimating temperature sensitivity on record with some freezing temperatures",{
			...
			expect_true(sum(isValid) >= 13 && sum(REddyProc:::isValidNightRecord(dss)) < nrow(dss) )
			#
			resE0 <- REddyProc:::partGLEstimateTempSensInBoundsE0Only(dss$NEE_f[isValid], dss$Temp[isValid]+273.15)
...
test_that("applyWindows",{
			...
			resApply <- REddyProc:::applyWindows( ds, fReportTime, prevRes, winSizeInDays=6L, nRecInDay=nRecInDay )	# larger than reference window of 4 days
...
test_that("simplifyApplyWindows",{
			...
			resApply <- REddyProc:::applyWindows( ds, fReportTimeSimple, winSizeInDays=6L, nRecInDay=nRecInDay )	# larger than reference window of 4 days
			...
			resApply <- REddyProc:::applyWindows( ds, fReportTimeSimpleDs, winSizeInDays=6L, nRecInDay=nRecInDay )	# larger than reference window of 4 days
...
test_that("estimating temperature sensitivity windows outputs are in accepted range",{
			...
			dss$NEE <- dss$NEE_f
			dss <- dss[ order(dss$Temp), ]
			res <- REddyProc:::simplifyApplyWindows( REddyProc:::applyWindows(dss, REddyProc:::partGLFitNightTempSensOneWindow, prevRes=data.frame(E0=NA)
					, winSizeInDays=12L
...
test_that("partGLFitLRCWindows outputs are in accepted range",{
			...
			dsTempSens <- dsTempSens0 <- REddyProc:::partGLFitNightTimeTRespSens( ds
					, nRecInDay=48L
					, controlGLPart=partGLControl()
			)
			lrcFitter <- RectangularLRCFitter()
			#lrcFitter <- NonrectangularLRCFitter()
			resFits <- REddyProc:::partGLFitLRCWindows(ds, nRecInDay=48L, dsTempSens=dsTempSens, controlGLPart=partGLControl(nBootUncertainty=10L), lrcFitter=lrcFitter)
			expect_true( var(resFits$k) > 0)
			expect_true( all( sapply(resFits$resOpt, length) != 0 ))
			.tmp.f <- function(){
				# in order to replicate, use nBoot=0L
				resParms0 <- REddyProc:::partGLFitLRCWindows(ds, nRecInDay=48L, dsTempSens=dsTempSens, controlGLPart=partGLControl(nBootUncertainty=0L), lrcFitter=lrcFitter)
...
testGLInterpolateFluxes <- function( lrcFitter, resEx ){
			tmp <- REddyProc:::partGLInterpolateFluxes( dsNEE$Rg_f, dsNEE$VPD_f, dsNEE$Temp, resEx, lrcFitter=lrcFitter)
			...
			tmp <- REddyProc:::partGLInterpolateFluxes( dsNEE$Rg_f, dsNEE$VPD_f, dsNEE$Temp, resEx
...
test_that("partitionNEEGL sparse data",{
			...
			resLRC <- REddyProc:::partGLFitLRCWindows(ds, dsTempSens=dsTempSens, lrcFitter=RectangularLRCFitter() )
...
test_that("partitionNEEGL isNeglectVPDEffect",{
			...
			#resLRC$nValidRec
			#
			resLRC2 <- REddyProc:::partGLFitLRCWindows(ds, dsTempSens=dsTempSens, lrcFitter=RectangularLRCFitter()
				,controlGLPart=ctrl  )
			...
			#resLRC2
			#
			resLRC2C <- REddyProc:::partGLFitLRCWindows(ds, dsTempSens=dsTempSens, lrcFitter=RectangularLRCFitterCVersion()
					,controlGLPart=ctrl  )
...
test_that("partGLPartitionFluxes missing night time data",{
			...
			resLRC <- REddyProc:::partGLFitLRCWindows(ds, lrcFitter=RectangularLRCFitter(), dsTempSens=dsTempSens )
