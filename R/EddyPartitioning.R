...
sEddyProc_sMRFluxPartition <- function(
  ...
  FluxVar.s = 'NEE_f'       ##<< Variable name of column with original and
  ...
  , QFTempValue.n = 0       ##<< Value of temperature quality flag for _good_
  ## (original) data
  , RadVar.s = 'Rg'         ##<< Unfilled (original) radiation variable
  , T_ref.n = 273.15 + 15     ##<< Reference temperature in Kelvin (degK)
  ## used in \code{fLloydTaylor} for regressing Flux and Temperature
  , suffix = ''		     ##<< String suffix needed for different processing
  ...
  fCheckColNames(
    cbind(.self$sDATA, .self$sTEMP)
    , c(FluxVar.s, QFFluxVar.s, TempVar.s, QFTempVar.s, RadVar.s)
    , 'sMRFluxPartition')
  fCheckColNum(
    cbind(.self$sDATA, .self$sTEMP)
    , c(FluxVar.s, QFFluxVar.s, TempVar.s, QFTempVar.s, RadVar.s)
  ...
  fCheckColPlausibility(
    ...
    , 'sMRFluxPartition')
  Var.V.n <- fSetQF(
    cbind(.self$sDATA, .self$sTEMP), FluxVar.s, QFFluxVar.s, QFFluxValue.n
    , 'sMRFluxPartition')

  message('Start flux partitioning for variable ', FluxVar.s
          , ' with temperature ', TempVar.s, '.')
  ...
  sTEMP$FP_VARnight <<- ifelse(
    sDATA[, RadVar.s] > 10 | .self$sTEMP$PotRad_NEW > 0, NA,  Var.V.n)
  ...
  sTEMP$FP_Temp_NEW <<- fSetQF(
    cbind(.self$sDATA, .self$sTEMP), TempVar.s, QFTempVar.s, QFTempValue.n
    , 'sMRFluxPartition')
  #
  ...
  sTEMP$E_0_NEW <<- do.call(.self$sRegrE0fromShortTerm, c(
    list('FP_VARnight', 'FP_Temp_NEW', T_ref.n = T_ref.n
         , CallFunction.s = 'sMRFluxPartition', debug.l = debug.l)
    , parsE0Regression))
  ...
  sTEMP$R_ref_NEW <<- .self$sRegrRref(
    'FP_VARnight', 'FP_Temp_NEW', 'E_0_NEW', T_ref.n = T_ref.n
    , CallFunction.s = 'sMRFluxPartition')
  #
  # Calculate the ecosystem respiration Reco
  sTEMP$Reco_NEW <<- fLloydTaylor(
    .self$sTEMP$R_ref_NEW, .self$sTEMP$E_0_NEW
    , fConvertCtoK(cbind(.self$sDATA, .self$sTEMP)[, TempVar.s])
    , T_ref.n = T_ref.n)
  ...
  sTEMP$GPP_NEW_f <<-
    -cbind(.self$sDATA, .self$sTEMP)[, FluxVar.s] + .self$sTEMP$Reco_NEW
  ...
  ## If a suffix is provided and if the defaults for FluxVar.s and QFFluxVar.s
  ...
  ## Currently, this works only with defaults of FluxVar.s = 'NEE_f'
  ## and QFFluxVar.s = 'NEE_fqc'.
  ## }}
...
sEddyProc_sCalcPotRadiation <- function(
  ...
  useSolartime.b = TRUE	##<< by default corrects hour (given in local winter time)
  ...
  sTEMP$PotRad_NEW <<- fCalcPotRadiation(
    ...
    , sLOCATION$TimeZone_h.n, useSolartime.b = useSolartime.b)
}
...
fOptimSingleE0 <- function(
  ...
  NEEnight.V.n	  ##<< (Original) nighttime ecosystem carbon flux, i.e.
  ## respiration vector
  , Temp_degK.V.n	##<< (Original) air or soil temperature vector (degC)
  , T_ref.n #= 273.15 + 15  ##<< Reference temperature in Kelvin (degK) used
  ...
  res <- tryCatch({
    ...
    NLS.L <- nls(
      formula = R_eco ~ fLloydTaylor(R_ref, E_0, Temp, T_ref.n = T_ref.n)
      , trace = FALSE
      , data = data.frame(R_eco = NEEnight.V.n, Temp = Temp_degK.V.n)
    ...
    t.b <- Residuals.V.n >= quantile(Residuals.V.n, probs = c(Trim.n / 100)) &
      Residuals.V.n <= quantile(Residuals.V.n, probs = c(1 - Trim.n / 100))
    # Trimmed non-linear regression
    NLS_trim.L <- nls(
      formula = R_eco ~ fLloydTaylor(R_ref, E_0, Temp, T_ref.n = T_ref.n)
      , algorithm = 'default', trace = FALSE
      , data = data.frame(R_eco = NEEnight.V.n[t.b], Temp = Temp_degK.V.n[t.b])
      , start = list(R_ref = 2, E_0 = 200))
...
fRegrE0fromShortTerm = function(
  ...
  NightFlux.V.n		    ##<< (Original) nighttime ecosystem carbon flux, i.e.
  ## respiration vector
...
## AMM
{
  ...
  NLSRes.F <- as.data.frame(do.call(
    rbind, NLSRes.l <- lapply(
      seq(WinDays.i + 1, max(DayCounter.V.i), DayStep.i), function(DayMiddle.i) {
    #TEST: DayMiddle.i <- 8
    ...
    #
    Subset.b <-
      DayCounter.V.i >= DayStart.i &
      DayCounter.V.i <= DayEnd.i &
      !is.na(NightFlux.V.n) &
      !is.na(TempVar.V.n)
    ...
    if (length(NEEnight.V.n) > MinData.n && diff(range(Temp_degK.V.n)) >= TempRange.n) {
      ...
      resOptim <- fOptim(
        NEEnight.V.n, Temp_degK.V.n, algorithm = optimAlgorithm, T_ref.n = T_ref.n)
      NLSRes.F <- c(
        Start = DayStart.i, End = DayEnd.i, Num = length(NEEnight.V.n)
        , TRange = diff(range(Temp_degK.V.n)),
        resOptim)
    } else NULL
  }) ))
  Limits.b <- (NLSRes.F$E_0_trim - NLSRes.F$E_0_trim_SD > MinE_0.n &
                 NLSRes.F$E_0_trim + NLSRes.F$E_0_trim_SD < MaxE_0.n)
...
sEddyProc_sRegrE0fromShortTerm <- function(
  ...
  NightFlux.s   ##<< Variable with (original) nighttime ecosystem carbon flux,
  ## i.e. respiration
  ...
  if ( (length(debug.l$fixedE0) != 0) && is.finite(debug.l$fixedE0) ) {
    ...
    message(
      'Using prescribed temperature sensitivity E_0 of: '
    ...
    E_0_trim.n <- fRegrE0fromShortTerm(
      NightFlux.V.n, TempVar.V.n, DayCounter.V.i, ...,
...
sEddyProc_sRegrRref <- function(
  ...
  ## i.e. respiration
  , TempVar.s     ##<< Variable with (original) air or soil temperature (degC)
  , E_0.s         ##<< Temperature sensitivity E_0 estimated with
  ...
  fCheckColNames(
    cbind(.self$sDATA, .self$sTEMP), c(NightFlux.s, TempVar.s, E_0.s), SubCallFunc.s)
  fCheckColNum(
    cbind(.self$sDATA, .self$sTEMP), c(NightFlux.s, TempVar.s, E_0.s), SubCallFunc.s)
  #
  # Regression settings
  ...
  for (DayMiddle.i in seq(WinDays.i + 1, max(DayCounter.V.i), DayStep.i)) {
    ...
    #
    Subset.b <- DayCounter.V.i >= DayStart.i &
      DayCounter.V.i <= DayEnd.i &
      !is.na(NightFlux.V.n) &
      !is.na(TempVar.V.n)
    ...
    if (length(NEEnight.V.n) > MinData.n) {
      ...
      tryCatch({
        LM.L <- lm(R_eco ~ 0 + fLloydTaylor(
          R_ref, E_0, Temp_degK, T_ref.n = T_ref.n)
          , data = data.frame(
            R_eco = NEEnight.V.n, R_ref = 1, E_0 = E_0.V.n
            , Temp_degK = Temp_degK.V.n))
        ...
        tmp.f <- function(x) {
          ...
          plot(NEEnight.V.n ~ fLloydTaylor(
            1, E_0.V.n, Temp_degK.V.n, T_ref.n = T_ref.n))
