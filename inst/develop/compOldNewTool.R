#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#+++ Developers' R script +++
#+++ Compare old and new tool and Fluxnet
#+++ It is important to perform the new algorithm on the same subset of data after filtering.
#+++ Therefore the new algorithm has to be performed AFTER the old pv-wave algorithm.
...
# 1. Generate files with data format of old PV-Wave tool using the script 'genOldToolData.R'
...
# 3. Load data from old online tool output file <..._DataSetafterFluxpart.txt>
...
#!!! source because of new function
source('~/Projects/REddy/REddyProc/R/FileHandling.R')

# Result files with old tool pv-wave output data
ResultsFile.V.s <- fInitFilesDir('out', 'DataSetafterFluxpart.txt')
ResultsName.V.s <- fStripFileExtension(SiteFile.V.s) #!!! Programm to be only AT-Neu...
ResultsName.V.s <- c('DE-Tha')
...
for (Result.i in 1:length(ResultName.V.s)) {
  ...
  # Load old tool pv-wave output data
  if( sum(grepl('OldToolData.F', ls(all=TRUE))) == 0 ) # Load only OldToolData.F, if not yet loaded
    OldToolData.F <- fLoadTXTIntoDataframe('Example_DETha98_PVWave_DataSetafterFluxpart.txt', 'inst/examples') #takes a minute or two
  
  # Convert data frame to comply with REddyProc format
  PVData.F <- fConvertTimeToPosix(OldToolData.F, 'YMDHM', Year.s = 'Year', Month.s= 'Month', Day.s = 'Day', Hour.s = 'Hour', Min.s = 'Minute')
  ...
  # Very basic plot of differences to BGI fluxnet version
  # Plot 1998 ...
